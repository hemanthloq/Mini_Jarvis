...
- **Free-tier rate limits** exist on every provider (per-minute token/request caps).
  The smart path handles this by **failing over to the next provider** the moment one is
  throttled, so a busy minute switches brains rather than stalling. Configure more than
  one provider in `.env` for the smoothest experience.
